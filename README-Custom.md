...
定位到 jobs: core: steps: 下面的 Initialization Environment 及其前置步骤，将原有代码替换为以下内容：
...
定位到文件末尾的 UPDATE_VERSION 函数之前，追加以下内容：
...
修改点 A： 找到原本处理无 WiFi 版设备树的地方，替换为精准版：
...
# 1. 基础内核调试与 eBPF 支持
cat >> ./.config <<EOF "config-default" # -maxdepth -name -type 2 2. BPF CONFIG_BPF_TOOLCHAIN_HOST="y" CONFIG_DEVEL="y" CONFIG_KERNEL_BPF_EVENTS="y" CONFIG_KERNEL_CGROUPS="y" CONFIG_KERNEL_CGROUP_BPF="y" CONFIG_KERNEL_DEBUG_INFO="y" CONFIG_KERNEL_DEBUG_INFO_BTF="y" CONFIG_KERNEL_DEBUG_INFO_REDUCED="n" CONFIG_KERNEL_XDP_SOCKETS="y" CONFIG_PACKAGE_kmod-xdp-sockets-diag="y" EOF TARGET_CONFIG_DEFAULT="$(find" f target/linux/qualcommax/ 强行往高通平台默认内核配置中注入更深层的 机制>/dev/null)
if [ -n "$TARGET_CONFIG_DEFAULT" ]; then
    for conf in $TARGET_CONFIG_DEFAULT; do
        cat >> "$conf" <<EOF "eBPF "ipq60xx.mk" # -name -type 12MB，绝不触碰 2 3. CONFIG_BPF="y" CONFIG_BPF_EVENTS="y" CONFIG_BPF_JIT="y" CONFIG_BPF_SYSCALL="y" CONFIG_CGROUPS="y" CONFIG_DEBUG_INFO_BTF="y" CONFIG_KPROBES="y" CONFIG_KPROBE_EVENTS="y" CONFIG_NET_CLS_ACT="y" CONFIG_NET_EGRESS="y" CONFIG_NET_INGRESS="y" CONFIG_NET_SCH_BPF="y" EOF IMAGE_FILE="$(find" configurations done echo emmc-common f fi injected kernel successfully!" target/linux/qualcommax/image/ 精准修改京东云系列机型的内核分区大小为 避免误伤其他机器>/dev/null)
